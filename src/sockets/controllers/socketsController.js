...
const { JWTVerify } = require('../../helpers/jwt-generator')
...
const socketController = (socket = new Socket(), io) => {
    ...
    socket.on('enterChat', async (data, callback) => {
        ...
        socket.broadcast.to(data.chatRoom.name).emit('newUser', createMessage(data.chatRoom.name, 'Admin', data.userName, `${data.userName} is connected!!`), connectedUsers);
    ...
    socket.on('createMessage', (data) => {
        ...
        if (userTo && sender) {
            if (userTo.userName == 'Everybody') io.to(userTo.chatRoom).emit('emitMessage', createMessage(data.chatRoom, data.from, data.to, data.msg, data.idSelected));
            else {
                socket.broadcast.to(userTo.id).emit('emitMessage', createMessage(data.chatRoom, data.from, data.to, data.msg, data.idSelected));
                socket.broadcast.to(sender.id).emit('emitMessage', createMessage(data.chatRoom, data.from, data.to, data.msg, data.idSender));
            }
        }
    });
    ...
    socket.on('disconnect', () => {
        ...
        if (deletedUser) {
            ...
            io.to(deletedUser.chatRoom).emit('userDisconnect', createMessage(deletedUser.chatRoom, 'Admin', deletedUser.userName, `${deletedUser.userName} has left the chat`), connectedUsers);
        }
    });
